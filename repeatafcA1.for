...
c      open(1,file='sosikidt',status='old')
     ...
     +'何県のデータを分析しますか'
      ...
      write(90,*) cp
      write(91,*) cp
      open(6,file='repeatT_afc2_'//cp//'.dat')
      write(*,*) '何頭以上を単独で扱うか？'
      read(*,*) nn
      akm='＊　　　　　　　　　　　　　　　　　　　'
      do while( .not.eof(1) )
     ...
     +                 or.akai(j)(1:2).ne.'＊')then
      ...
      do i=1,nk
        ...
        if( numc(ip).lt.nn ) then
          ...
          newc(i)=iken*1000000+900000
          kk=num2(i)
          call hash(newc(i),ip,neqn,count )
          numc2(ip)=numc2(ip)+kk
c          write(4,'(a14,i6,i7,x,a40,i9)') 
c     +   oldc(i),numc2(ip),kaik(i),akaik(i),newc(i)
          go to 101
        end if
          ...
          if( bp(i).eq.fm(1:2) ) then
            do j=lp(i),lp(i+1)-1
              if( oldc(j).eq.fm ) then
                write(6,'(a49,i3,i9,a30)') zen,ipa,newc(j),kou(1:30)
                go to 100
              end if
            end do
          end if
...
c        write(6,'(a49,i3,x,a8,a30)') zen,ipa,fm(1:2)//'900000',kou(1:30)
        ...
        do i=1,ns
          if( qfarm(i).eq.fm ) then
            write(6,'(a49,i3,x,a8,a30)') 
     +       zen,ipa,fm(1:2)//'900000',kou(1:30)
            go to 100
          end if
        end do
        ns=ns+1
        ...
        if( ick(ip).eq.0 ) then 
          iw1=48900000
          write(6,'(a49,i3,i9,a30)') zen,ipa,iw1,kou(1:30)
          write(4,'(a14,i6,x,a6,x,20a2,i9)') 
     +   fm,numc(ip),fm(1:2)//'0000',("　",j=1,20),iw1
          go to 100
        end if
        write(6,'(a49,i3,x,a8,a30)') zen,ipa,fm(1:2)//'900000',kou(1:30)
        write(4,'(a14,i6,x,a6,x,20a2,i9)') 
     +   fm,numc(ip),fm(1:2)//'0000',("　",j=1,20),iw1
100     cycle
      end do
