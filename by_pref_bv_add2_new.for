...
c          call rounda(rwv2(j))
     ...
     +   id,iw1,((rwv(j),rwv2(j)),j=1,2),(iwv(j),j=1,2)
        call hash( id,ip,neqn,count )
        do j=1,2!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!記録を持つ後代のいないものを期待育種価にしていたが201910評価よりこのルーチンを削除したのを202110より復活させる。
          if( iflag(ip).eq.0.and.iwv(j).eq.0 ) then
            ...
            ikigo(ip,j)=1
          end if!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!記録を持つ後代のいないものを期待育種価にしていたが201910評価よりこのルーチンを削除したのを202110より復活させる。
        end do
