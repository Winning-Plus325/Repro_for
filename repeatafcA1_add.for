...
c          newf(ip,4)=48*1000000+900000
          if( nnf(ip,1).le.9.or.nnf(ip,2).le.9 ) then
            newf2(ip)=0
        ...
        if( ifarm.eq.1100334 ) write(*,*) 1100334,newf2(ip),nnf(ip,2)
        if( ifarm.eq.1100336 ) write(*,*) 1100336,newf2(ip),nnf(ip,2)
      end do
      call hash( 1100334,ip,neqn,count )
...
c          newf(ip,4)=48*1000000+900000
          if( nnf2(ip,1).le.9.or.nnf2(ip,2).le.9 ) then
      ...
      do while( .not.eof(3) )
        ...
        if( iw2.eq.9 ) then !県単位Ｇ
          ...
          if( nnf3(ip,1).le.9.or.nnf3(ip,2).le.9 ) then
            newf2(ip)=0
     ...
     +        aa14,num,iw1,aa40,(newf4(ip3),j=1,2),(nnf4(ip4,j),j=1,2)
              ...
              if( nnf4(ip3,1).le.9.or.nnf4(ip3,2).le.9 ) then
     ...
     +        aa14,num,iw1,aa40,(newf3(ip2),j=1,2),(nnf4(ip3,j),j=1,2)
            ...
            if( nnf4(ip2,1).le.9.or.nnf4(ip2,2).le.9 ) then
              if( iflag2(ip).eq.1 ) cycle
              iflag2(ip)=1
     ...
     +    aa14,num,iw1,aa40,(newf4(ip),j=1,2),(nnf4(ip2,j),j=1,2)
      ...
      do while( .not.eof(3) )
        ...
        if( nnf5(ip2,1).le.9.or.nnf5(ip2,2).le.9 ) then
     ...
     +  (nnf5(ip2,j),j=1,2)
      ...
      do while( .not.eof(1) )
        ...
        if( honf(ip).eq.0 ) honf(ip)=48*1000000+900000
